...
class enemy(tank):
    def __init__(self, x, y, w, h, image, image2, speed , hp):
        ...
        angle = pygame.Vector2(0, -10).angle_to(vect2)
        print(angle)
    ...
    def enemy_move(self):
        ...
        if not self.rect.colliderect(base.rect):
            ...
            self.image2 = pygame.transform.rotate(self.image2 , -angle)
            self.rect = self.image1.get_rect(center=self.rect.center)
            # loc = self.image1.get_rect().center
            # if angle < 0:
            #     angle = 360+angle
            ...
            # elif angle >= 160 and angle < 250:
            #     self.image1 = pygame.transform.rotate(self.image1 , 180)
            #     self.image2 = pygame.transform.rotate(self.image2 , 180)
            # elif angle >= 250 and angle < 340:
            #     self.image1 = pygame.transform.rotate(self.image1 , 270)
            #     self.image2 = pygame.transform.rotate(self.image2 , 270)
            # self.rect = self.image1.get_rect(center=self.rect.center)
            # print(self.rect.x , self.rect.y)
            # print(angle)
        else :
            tank_list.remove(self)
            enemy(randint(0, 900),randint(0, 700),50,50,tank_track1 , tank_corp1 , 5, 3)
...
tank1 = enemy(randint(0, 900),randint(0, 700),50,50,tank_track1 , tank_corp1 , 5, 3)
game = True
blocks = []
x,y = 0,0
FPS = 120
